...
class Solution:
    def recursive_sebsets(self, nums, index=0):
        for i in range(index, len(nums)):
            ...
            self.recursive_sebsets(nums, i + 1)
            self.subset.pop()
    ...
    def subsetsWithDup(self, nums: List[int]) -> List[List[int]]:
        ...
        self.subset = []
        self.subsets = set()
        self.recursive_sebsets(nums)
        return self.result

    def recursive_sebsets2(self, nums, result, subset=[], index=0):
        for i in range(index, len(nums)):
            if i != index and nums[i] == nums[i - 1]:
                continue
            subset.append(nums[i])
            result.append(subset[:])
            self.recursive_sebsets2(nums, result, subset, i + 1)
            subset.pop()
    ...
    def subsetsWithDup2(self, nums: List[int]) -> List[List[int]]:
        ...
        nums.sort()
        result = [[]]
        self.recursive_sebsets2(nums, result)
        return result
